...
const float defaultCutOff = 0.75f;
const float defaultLowPassFreq = 3.0f;
	  ...
	  lowPassBuffer(2, 12000)
{
	...
	lowPassFreq = defaultLowPassFreq;
...
float JuceDemoPluginAudioProcessor::getParameter (int index)
{
    ...
    switch (index)
    {
		...
		case lowPassFreqParam:	return lowPassFreq;
        default:				return 0.0f;
    }
}
...
void JuceDemoPluginAudioProcessor::setParameter (int index, float newValue)
{
    ...
    switch (index)
    {
        case cutOffParam:		cutOff = newValue;  break;
        case delayParam:		delay = newValue;  break;
		case lowPassFreqParam:	lowPassFreq = newValue; break;
        default:				break;
    }
}
...
float JuceDemoPluginAudioProcessor::getParameterDefaultValue (int index)
{
    switch (index)
    {
		...
		case lowPassFreqParam:	return defaultLowPassFreq;
        default:				break;
    }
...
void JuceDemoPluginAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    for (channel = 0; channel < getNumInputChannels(); ++channel)
        buffer.applyGain (channel, 0, buffer.getNumSamples(), cutOff);*/
    ...
    for (channel = 0; channel < getNumInputChannels(); ++channel)
    {
		...
		dp = delayPosition;
		lpp = lowPassPosition;
		angle = float_Pi * 2 / getSampleRate() * lowPassFreq;
		
        for (int i = 0; i < numSamples; ++i)
        {
			if (channelData[i] > cutOff * max)
			{
				channelData[i] = cutOff * max;
			}

			float cmax = FLT_MIN;
			if (channelData[i] > cmax)
			{
				cmax = channelData[i];
				channelData[i] *= 0.25;
